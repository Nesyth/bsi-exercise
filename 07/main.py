...
swears = ['chuj','chuja', 'chujek', 'chuju', 'chujem', 'chujnia',
...
for attempt in bruteforce(dictionary, 12):
	des = DES.new(bytes(attempt, encoding = "UTF-8"), DES.MODE_ECB)
	decryptedText = des.decrypt(encryptedText)
	for i in swears:
		if bytes(i, encoding = "UTF-8") not in decryptedText:
			continue
		else:
			print('DECRYPTED TEXT: ' + str(decryptedText, encoding = "UTF-8") + '\n')
			print('PASSWORD: ' + bytes(attempt, encoding = "UTF-8") + '\n')
